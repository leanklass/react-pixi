  ...
  transferDisplayObjectPropsByName(oldProps, newProps, propsToCheck) {
    ...
    for (var propname in propsToCheck) {
      if (typeof newProps[propname] !== "undefined") {
        ...
        typeof oldProps[propname] !== "undefined" &&
...
var DisplayObjectContainerMixin = assign(
  {},
  DisplayObjectMixin,
    ...
    mountAndAddChildren: function(children, transaction, context) {
      ...
      for (var key in this._renderedChildren) {
        if (this._renderedChildren.hasOwnProperty(key)) {
          ...
          DisplayObjectContainerMixin.createChild.call(
            this,
            child,
            prevNode,
            mountedImages[i]
...
var PIXIStage = React.createClass({
  ...
  generateDefaultRenderer: function(props) {
    ...
    let backgroundColor = typeof props.backgroundColor === "number"
    ...
    if (typeof props.backgroundcolor !== "undefined") {
      warning(
        false,
        "Using lowercase on Stage prop backgroundcolor is deprecated - use backgroundColor instead"
      );
      backgroundColor = props.backgroundcolor;
    }
    const transparent = props.transparent ? props.transparent : false;
    const antialias = props.antialias ? props.antialias : false;
    const preserveDrawingBuffer = props.preserveDrawingBuffer
      ? props.preserveDrawingBuffer
      : false;
    const resolution = typeof props.resolution === "number"
      ? props.resolution
      : 1;
  ...
  componentDidUpdate: function(oldProps) {
    ...
    if (
      newProps.renderer !== oldProps.renderer ||
...
var NullDisplayObject = createPIXIComponent(
  ...
  CommonDisplayObjectContainerImplementation,
  {
    ...
    mountComponent: function(
      ...
      if (this._nativeParent instanceof ReactDOMComponent) {
        var nodeValue =
          " react-empty: " + this._nativeContainerInfo._idCounter++ + " ";
...
var SpriteComponentMixin = {
  createDisplayObject: function() {
    if (this._currentElement.props.image) {
      ...
      warning(
        texture instanceof PIXI.Texture,
        "the Sprite 'texture' prop must be an instance of PIXI.Texture"
      );
  ...
  applySpecificDisplayObjectProps: function(oldProps, newProps) {
    ...
    if (
      typeof newProps.image !== "undefined" &&
      newProps.image !== oldProps.image
    ) {
      displayObject.texture = PIXI.Texture.fromImage(newProps.image);
    } else if (
      typeof newProps.texture !== "undefined" &&
      newProps.texture !== oldProps.texture
    ) {
      warning(
        newProps.texture instanceof PIXI.Texture,
        "the Sprite 'texture' prop must be an instance of PIXI.Texture"
      );
...
var TilingSpriteComponentMixin = {
  createDisplayObject: function() {
    ...
    return new PIXI.extras.TilingSprite(
      PIXI.Texture.fromImage(props.image),
      props.width,
      props.height
    );
  },
...
var TextComponentMixin = {
  ...
  applySpecificDisplayObjectProps: function(oldProps, newProps) {
    ...
    if (
      typeof newProps.style !== "undefined" &&
...
var BitmapTextComponentMixin = {
  ...
  applySpecificDisplayObjectProps: function(oldProps, newProps) {
    ...
    if (
      ...
      if (
        typeof oldProps.style === "undefined" ||
        newProps.style.font !== oldProps.style.font
      ) {
...
var CustomDisplayObjectImplementation = {
  mountComponent: function(rootID, transaction, context) {
    let props = this._currentElement.props;
    warning(
      this.customDisplayObject,
      "No customDisplayObject method found for a CustomPIXIComponent"
    );
